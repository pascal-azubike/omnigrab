...
<div class="w-full max-w-3xl mx-auto space-y-4">
  <form onsubmit={handleSubmit} class="relative group">
    ...
    >
      ...
      {:else}
        <Search
          class="h-5 w-5 text-muted-foreground group-focus-within:text-indigo-400 transition-colors"
        />
      {/if}
    </div>
    ...
    <input
      ...
      class="block w-full pl-12 pr-4 py-4 bg-card/50 border border-border rounded-2xl
             text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/50
    ...
    <button
      ...
      class="absolute right-2 top-2 bottom-2 px-6 bg-indigo-600 hover:bg-indigo-500
             disabled:bg-accent disabled:text-muted-foreground text-white font-semibold
  ...
  {#if error}
    <div
      class="bg-red-500/10 border border-red-500/20 text-red-500 px-4 py-3 rounded-xl text-sm animate-in fade-in slide-in-from-top-2"
    >
      {error}
    </div>
  {/if}
